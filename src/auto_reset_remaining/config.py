...
def _required_table(document: tomlkit.TOMLDocument, name: str) -> Table:
...
def _required_str(table: Table, key: str, field_name: str) -> str:
...
def _str_value(table: Table, key: str, fallback: str) -> str:
...
def _int_value(table: Table, key: str, fallback: int, field_name: str) -> int:
...
def _float_value(table: Table, key: str, fallback: float, field_name: str) -> float:
    value: Any = table.get(key, fallback)
    try:
        return float(value)
    except (TypeError, ValueError) as exc:
...
def _bool_value(table: Table, key: str, fallback: bool, field_name: str) -> bool:
    value: Any = table.get(key, fallback)
